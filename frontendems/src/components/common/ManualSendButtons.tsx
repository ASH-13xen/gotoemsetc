...
export function ManualSendButtons({
  ...
  whatsappHref?: string
  storageKey?: string
  // Fired right before the mailto/wa.me link navigates — used e.g. to
  // trigger a file download so it's sitting in Downloads ready for the
  // admin to manually attach (mailto/wa.me links can't attach files
  // themselves, there's no such thing as a URL-based attachment).
  onEmailClick?: () => void
  onWhatsappClick?: () => void
  // Controlled mode — pass both to let a parent own the "notified" flag
  // (e.g. so it stays in sync with a checkbox shown elsewhere for the same
  // item). Falls back to managing its own state off storageKey when omitted.
  checked?: boolean
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mt-2">
      <div className="flex flex-wrap gap-3">
        {emailHref && (
          <Button asChild className="h-10 shrink-0 gap-2 bg-[#EA4335] px-5 text-white hover:bg-[#EA4335]/90">
        ...
        {whatsappHref && (
          <Button asChild className="h-10 shrink-0 gap-2 bg-[#25D366] px-5 text-white hover:bg-[#25D366]/90">
