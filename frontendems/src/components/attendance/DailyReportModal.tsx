...
const WARNABLE_CATEGORIES: WarningCategory[] = ['late', 'early_departure', 'half_day', 'short_leave', 'absent']
      ...
      <DialogContent className="max-h-[90vh] w-full max-w-[95vw] overflow-y-auto sm:max-w-[95vw]">
        ...
        {report && report.recentWorkingDays.length > 1 && (
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Working day:</span>
                  ...
                  <TableBody>
                    {report.present_overtime.map((row) => (
                      <TableRow key={row.employee._id}>
                        ...
                        <TableCell>{row.record.overtimeHours}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
