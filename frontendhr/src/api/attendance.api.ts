...
export interface MonthlyOverviewRecord {
  _id: string
  date: string
  status?: AttendanceStatus
  overtimeMinutes: number
...
// every employee's records for one calendar month, flat (not pre-grouped by
// date — grouping happens client-side in AttendanceOverviewPage).
export async function listMonthlyOverview(month: number, year: number): Promise<{ records: MonthlyOverviewRecord[] }> {
  const { data } = await apiClient.get('/attendance/monthly-overview', { params: { month, year } })
  return data
}
