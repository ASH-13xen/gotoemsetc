...
router.get('/', requireRole(USER_ROLES.ADMIN), userController.list);
router.get(
...
router.get('/:id', requireRole(USER_ROLES.ADMIN), validate(userValidator.getById), userController.getById);
router.patch(
  '/:id',
  requirePermission(PERMISSIONS.ADD_CREDENTIALS),
...
router.delete('/:id', requireRole(USER_ROLES.ADMIN), validate(userValidator.removeCredential), userController.removeCredential);
